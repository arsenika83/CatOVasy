...
@export var max_defence = 15
var defended = false

@export var accuracy = 65
...
var standard_move_path : Array[Vector2] = [Vector2(0, 0)]
@export var step_count = 0

var move_set : Array = ["deal_damage", "defend", "buff"]

var buff_set : Array = [["strength", 1, 2]]
...
func _on_take_damage_timer_timeout() -> void:
	...
	var tween = create_tween()
	tween.tween_property(head_sprite.material, "shader_parameter/speed", 3.0, 0.2)
	tween.tween_property(head_sprite.material, "shader_parameter/speed", 2.0, 0.2)
	
	display_damage(taken_damage)
	
	if not state == "dead":
		sprite.play("take_damage")
		head_sprite.play("take_damage")
		audio_hurt.play()
...
func _on_deal_damage_timer_timeout() -> void:
	...
	if is_hit_lucky:
		get_parent().get_parent().player_camera.apply_shake(0.1 + shake)
