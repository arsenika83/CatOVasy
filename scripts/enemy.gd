...
func move(g_pos : Vector2, e_pos : Vector2) -> void:
	if state == "idle":
		...
		if abs(diff_x) > follow_radius or abs(diff_y) > follow_radius:
			...
			tween.tween_property(self, "position", Vector2(self.position.x + standard_move_path.get(step_count).x, self.position.y + standard_move_path.get(step_count).y), 0.2)
